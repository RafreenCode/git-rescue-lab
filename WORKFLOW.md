...
## The old secret
Ignoring `.env` and removing it from tracking keeps it out of future commits, but it is still in the old commits. To really remove it, I would rewrite the repository history with something like `git filter-repo` or BFG process, replace the credentials, and force-push the rewritten branches and tags. Everyone with a clone would need to coordinate that change. This assignment did not require it because rewriting shared history would add risk beyond the requested tracking cleanup.
